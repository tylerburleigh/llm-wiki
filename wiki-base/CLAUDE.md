...
If the source is not already markdown, convert it first. The converter
pattern (see "Source converters" in Wiki Conventions): produce a `.md`
sibling next to the original in `raw/`, leave the original immutable,
set the source-summary `raw_path` to the original, and ingest from the
converted markdown. `pymupdf4llm` is the shipped PDF converter; register
new converters for other source types (HTML, EPUB, audio transcripts)
